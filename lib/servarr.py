...
from lib.constants import Servarr, Radarr, Sonarr, ID_TYPE
from lib.tools import synchronized


ARR_ID_TYPE = f"{ID_TYPE}Id"
...
def get_arr_media_ids(arr_type: int) -> list:
    ...
    return list(map(lambda x: str(x.get(ARR_ID_TYPE)), j_res))
...
def check_new(arr_type: int, plex_media_ids: dict) -> bool:
    media_id: str
    title: str
    ...
    arr_media_ids: list = get_arr_media_ids(arr_type)
    logger.debug(
        "Plex %ss watchlist: %s", plex_type, plex_media_ids.get(plex_type)
    )
    for media_id, title in plex_media_ids[plex_type].items():
        if media_id not in arr_media_ids:
...
@synchronized
def radarr_check_new(plex_media_ids: dict) -> bool:
    return check_new(Radarr.TYPE, plex_media_ids)


@synchronized
def sonarr_check_new(plex_media_ids: dict) -> bool:
    return check_new(Sonarr.TYPE, plex_media_ids)


# shortcuts
def get_raddar_media_ids() -> list:
    return get_arr_media_ids(Radarr.TYPE)


def get_sonarr_media_ids() -> list:
    return get_arr_media_ids(Sonarr.TYPE)
